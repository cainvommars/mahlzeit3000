  ...
  return {
    ...
    event: function(req, res) {
      Event(db).retreive(req.params.id, function(err, data) {
        var msg = Session(req).getAndRemove('stash');
        res.render('event', {event: data, msg: msg});
      });
    },
    ...
    create_event: function(req, res) {
      ...
      Event(db).create(event, function(err, data) {
        ...
        s.set('stash', 'You successfully created the new lunch date.');
        res.send(data);
      });
    },
    ...
    join_event: function(req, res) {
      if (!req.params.hash) {
        renderError(res,'You are not allowed to join this lunch date.');
        return;
      }
      ...
      Event(db).retreive(req.params.id, function(err, data) {
        ...
        var pos = data.users.indexOf(viewer);
        data.users.splice(pos, 1);
    ...
    do_join_event: function(req, res, join) {
      ...
      Event(db).retreive(req.params.id, function(err, data) {
        ...
        if (!viewer) {
          res.send(404,'You are not allowed to join this lunch date.');
          return;
        }
        ...
        if (join) {
          viewer.status = 'join';
          s.set('stash', 'You successfully joined the lunch date.');
        } else {
          viewer.status = 'decline';
          s.set('stash', 'You successfully declined the lunch date.');
        }
